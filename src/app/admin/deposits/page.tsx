...
export default function AdminDepositsPage() {
  ...
  useEffect(() => {
    return onSnapshot(collection(db, "deposits"), (snap) => {
      setDeposits(snap.docs.map((d) => ({ id: d.id, ...d.data() })).sort((a, b) => {
  ...
  const filteredDeposits = deposits.filter((d) =>
    `${d.userEmail} ${d.txHash} ${d.id}`.toLowerCase().includes(search.toLowerCase())
  );
      ...
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 4 }}>
        <Box>
          ...
          <Typography variant="body2" sx={{ color: "var(--muted-foreground)" }}>
            Review and process manual deposit verifications from platform investors.
          </Typography>
        </Box>
      </Stack>
        ...
        <Table>
          <TableHead sx={{ bgcolor: "rgba(255,255,255,0.02)" }}>
            <TableRow>
              ...
              <TableCell sx={{ color: "var(--muted-foreground)", fontWeight: 600 }}>Asset/Network</TableCell>
              <TableCell sx={{ color: "var(--muted-foreground)", fontWeight: 600 }}>Amount</TableCell>
              <TableCell sx={{ color: "var(--muted-foreground)", fontWeight: 600 }}>Tx Hash</TableCell>
              <TableCell sx={{ color: "var(--muted-foreground)", fontWeight: 600 }}>Proof</TableCell>
              <TableCell sx={{ color: "var(--muted-foreground)", fontWeight: 600 }}>Status</TableCell>
              <TableCell align="right" sx={{ color: "var(--muted-foreground)", fontWeight: 600 }}>Action</TableCell>
            </TableRow>
          </TableHead>
              ...
              filteredDeposits.map((d) => (
                <TableRow key={d.id} sx={{ "&:hover": { bgcolor: "rgba(255,255,255,0.01)" } }}>
                  ...
                  <TableCell>
                    ...
                    <Typography variant="caption" sx={{ color: "var(--muted-foreground)", textTransform: "uppercase" }}>
                      {d.network}
                    </Typography>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    {d.proofUrl ? (
                      <Button
                        ...
                        sx={{ borderRadius: 2, py: 0.5, textTransform: "none", fontSize: "0.75rem" }}
                      >
                        View Proof
                      </Button>
                    ) : (
                      <Typography variant="caption" sx={{ color: "var(--muted-foreground)" }}>No File</Typography>
                    )}
                  </TableCell>
                  <TableCell>
                    <Chip
                      label={d.status}
                      size="small"
                      icon={d.status === "approved" ? <CheckCircle2 size={12} /> : d.status === "rejected" ? <XCircle size={12} /> : <Clock size={12} />}
                      sx={{
                        bgcolor: d.status === "approved" ? "rgba(34, 197, 94, 0.1)" : d.status === "rejected" ? "rgba(239, 68, 68, 0.1)" : "rgba(234, 179, 8, 0.1)",
                  ...
                  <TableCell align="right">
                    {d.status === "pending" ? (
                      <Stack direction="row" spacing={1} justifyContent="flex-end">
                        ...
                        >
                          {loadingAction === d.id ? <CircularProgress size={16} color="inherit" /> : "Approve"}
                        </Button>
                        <Button
                          size="small"
                          color="error"
                        ...
                        >
                          {loadingAction === d.id ? <CircularProgress size={16} color="inherit" /> : "Reject"}
                        </Button>
                      </Stack>
                    ) : (
                      <Typography variant="caption" sx={{ color: "var(--muted-foreground)" }}>
